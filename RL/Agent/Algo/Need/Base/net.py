...
class Actor(nn.Module):
    ...
    """
    def __init__(self, mid_dim, state_dim, action_dim, cd2_dim):
        super().__init__()
        self.net = nn.Sequential(torch.nn.Conv2d(cd2_dim, mid_dim, kernel_size=(3,3), stride=(1,2), padding=1),
                                 torch.nn.Conv2d(mid_dim, mid_dim, kernel_size=(3,3), stride=(1,2), padding=1),
                                 torch.nn.Conv2d(mid_dim, 1, kernel_size=(3, 3), stride=(1, 1), padding=1))
        self.dnn = nn.Sequential(nn.Linear(state_dim, mid_dim), nn.ReLU(),
    ...
    def forward(self, state):
        ...
        if state.shape[0]<2048:
            results = self.net(state)
            output = torch.flatten(results)
            return self.dnn(output).tanh()  # action.tanh()
        else:
...
class Critic(nn.Module):
    ...
    def __init__(self, mid_dim, state_dim, action_dim, cd2_dim):
        ...
        self.net = nn.Sequential(torch.nn.Conv2d(cd2_dim, mid_dim, kernel_size=(3,3), stride=(1,2), padding=1),
                                 torch.nn.Conv2d(mid_dim, mid_dim, kernel_size=(3,3), stride=(1,2), padding=1),
                                 torch.nn.Conv2d(mid_dim, 1, kernel_size=(3, 3), stride=(1, 1), padding=1))
        self.dnn = nn.Sequential(nn.Linear(state_dim + action_dim, mid_dim), nn.ReLU(),
                                 nn.Linear(mid_dim, mid_dim), nn.ReLU(),
                                 nn.Linear(mid_dim, mid_dim), nn.Hardswish(),
                                 nn.Linear(mid_dim, 1))
    ...
    def forward(self, state, action):
        ...
        if state.shape[0]<2048:
            results = self.net(state)
            output = torch.flatten(results)
            return self.dnn(torch.cat((output, action), dim=1))  # Q value
        else:
            state = state.mean(dim=0)
            results = self.net(state)
            output = torch.flatten(results)
            if action.shape[0]<2048:
                return self.dnn(torch.cat((output, action), dim=0))  # Q value
            else:
