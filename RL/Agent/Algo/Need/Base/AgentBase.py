...
class AgentBase:  # [ElegantRL.2021.11.11]
    def __init__(self, net_dim=256, state_dim=8, action_dim=2, reward_scale=1.0, gamma=0.99, cd2_dim=30,
    ...
    def init(self, net_dim=256, state_dim=8, action_dim=2, reward_scale=1.0, gamma=0.99,cd2_dim=30,
             learning_rate=1e-4, if_per_or_gae=False, env_num=1, gpu_id=0):
        ...
        self.cri = self.ClassCri(int(net_dim * 1.25), state_dim, action_dim, cd2_dim).to(self.device)
        self.act = self.ClassAct(net_dim, state_dim, action_dim, cd2_dim).to(self.device) if self.ClassAct else self.cri
    ...
    def explore_one_env(self, env, target_step: int) -> list:
        ...
        for _ in range(target_step):
            state = state.reshape((state.shape[0], state.shape[1], 1, 1))
            ten_state = torch.as_tensor(state, dtype=torch.float32)
            ten_action = self.select_actions(ten_state)[0]
            action = ten_action.numpy()
            next_s, reward, done, _ = env.step(action)

            ten_other = torch.empty(2 + self.action_dim)
            ten_other[0] = torch.as_tensor(reward, dtype=torch.float32).mean()
            ten_other[1] = done
